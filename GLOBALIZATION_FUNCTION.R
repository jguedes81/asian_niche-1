...
globalization <- function(min_lon, max_lon){
  ...
  setwd("/data/kamiak/nick.maggio/JadeDAlpoim/dev/R")
  ...
  ### NICK COMMENTED OUT #####ETOPO5.rast <- raster("../GLOBALIZATION_ETOPO5.tif")
  ### cat("Topo Raster read in\n")

  #TESTING
  # ETOPO5.rast <- crop(ETOPO5.rast,c(100,101,50,51))
  ...
  crops <- crops[grep("ETOPO5", crops)]
  for(Crop in crops){
    Crop.name <- basename(Crop)
    Crop.name <- gsub(output_suffix,"",Crop.name)
    if(all(file.exists(paste0("../OUTPUT/MARCOTT/",Crop.name,"_",c("Z_Lower","Z","Z_Upper"),"_ETOPO5.tif")))) next
    crop.data <- readRDS(Crop)
    for(Zs in c("Z_Lower","Z","Z_Upper")){
      if(file.exists(paste0("../OUTPUT/MARCOTT/",Crop.name,"_",Zs,"_ETOPO5.tif"))) next
      ...
      writeRaster(out.rast,paste0("../OUTPUT/MARCOTT/",Crop.name,"_",Zs,"_ETOPO5.tif"), datatype="INT2U", options=c("COMPRESS=DEFLATE", "ZLEVEL=9", "INTERLEAVE=BAND"),overwrite=T,setStatistics=FALSE)
      print(out.rast)
    }
  }
  cat("Niche calculated and saved\n")
