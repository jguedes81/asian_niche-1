...
*Be sure to copy the archaeological site data into the `asian_niche/DATA/` directory prior to running!* You can do so by adding the archaeological site data to your local `asian_niche/DATA/` directory, commenting the `DALPOIMGUEDES_BOCINSKY_2017.xlsx` entry in the `.dockerignore` file, and rebuilding the docker image using the command below.
...
### Running everything using the `asian_niche.sh` script
Alternatively, you can run the `asian_niche.sh` script, which builds the Docker container, runs the analysis, archives the output and Docker image for Zenodo archiving, and outputs all of the supplemental information included in the d'Alpoim Guedes and Bocinsky (2017) paper. Do so using bash:
